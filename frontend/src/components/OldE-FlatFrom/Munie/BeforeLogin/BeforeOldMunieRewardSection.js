...
const BeforeOldMunieRewardSection = () => {
    const dispatch = useDispatch();
    const [checkChainId, setCheckChainId] = useState('');
    const { account } = useSelector((state) => state.account);
    const { oldMunieAmountStaked } = useSelector((state) => state.oldMunieStakingView);
    const { oldMunieResultValue, getOldMunieUnClaimedRewards, getOldMunieTotalReward } = useSelector(
        (state) => state.oldMunieStakingResult
    );
...
export default BeforeOldMunieRewardSection;
